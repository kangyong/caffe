...
//    vector<int> label_shape(1, batch_size);
      ...
      label_shape.push_back(batch_size);//batch_size
      label_shape.push_back(4);//channels
      label_shape.push_back(1);//height?
      label_shape.push_back(1);//width?
      top[1]->Reshape(label_shape);
      LOG(INFO) << "label shape size: " << top[1]->num() << ","
...
//    trans_time += timer.MicroSeconds();
    // modify-kangy
    for(int i = 0; i < datum.lf_label_size(); ++i){
        top_label[item_id * datum.lf_label_size() +i] = datum.lf_label(i);
        if(datum.lf_label(i) > 1.01){
